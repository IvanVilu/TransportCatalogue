  ...
  //  LOG_DURATION("main");
    ...
    vector<Query> o_queries = ReadInput(cin);
        ...
        if(query.type == QueryType::GetBusInfo) {
            PrintBusInfo(catalogue.GetBusInfo(ParseBusInfoQuery(query.request)));
        } else { 
            PrintStopInfo(catalogue.GetStopInfo(ParseStopInfoQuery(query.request)));
        }
...
// https://pastebin.com/r2zSUzdG
// https://pastebin.com/vtcjiqgj
